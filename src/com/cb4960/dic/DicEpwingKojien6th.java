...
/**
 * Represents 『広辞苑第六版』 (Kojien 6th Edition). J-J. Japanese-only example sentences.
 * 
 * Ported from JGlossator.
 */
public class DicEpwingKojien6th extends DicEpwing
{
  /** Constructor. */
  ...
  public List<Entry> lookup(String word, boolean includeExamples, FineTune fineTune)
  {
    ...
    List<String> rawEntryList = this.searchEpwingDic(word, EPLKUP_TAG_FLAG_LINK | EPLKUP_TAG_FLAG_KEYWORD
        | EPLKUP_TAG_FLAG_SUB | EPLKUP_TAG_FLAG_SUP);
   ...
   * kanji entries and E-J entries (by design). Returns false on error.
   */
  private boolean parseFirstLine(String line, Entry entry)
  {
    ...
    {
      // .Net: ^<KEYWORD>○(?<Expression>.*?)</KEYWORD>.*?<sub>(?<Reading>.*?)</sub>
      pattern = Pattern.compile("^<KEYWORD>○(.*?)</KEYWORD>.*?<sub>(.*?)</sub>");
      matcher = pattern.matcher(line);
    }
    else
    {
      // .Net: ^<KEYWORD>(?<Reading>.*?)</KEYWORD>(?:【<KEYWORD>(?<Expression>.*?)</KEYWORD>】)?
      pattern = Pattern.compile("^<KEYWORD>(.*?)</KEYWORD>(?:【<KEYWORD>(.*?)</KEYWORD>】)?");
      matcher = pattern.matcher(line);
    }
    ...
    if (matcher.find())
    {
      ...
      if ((entry.Reading.length() > 0)
          && UtilsLang.containsAlpha(UtilsLang.convertJapAlphaToAscii(entry.Reading.charAt(0))))
      {
        success = false;
      }
      else
