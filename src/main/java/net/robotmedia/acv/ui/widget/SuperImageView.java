...
public class SuperImageView extends ImageView
{
  ...
  public void computeScroll()
  {
    if (mScroller.computeScrollOffset())
    {
      ...
      // Log.d("computeScroll", "scrollX =" + String.valueOf(x) + "; scrollY = " +
      // String.valueOf(y));
      super.scrollTo(x, y);
  ...
  {
    mScroller.fling(getScrollX(), getScrollY(), initialVelocityX, initialVelocityY, 0, getWidth() - getRootViewWidth(),
        0, getHeight() - getRootViewHeight());
    invalidate();
    return true;
  }
  ...
  private float getTheMaxWidth()
  {
    SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getContext());
    return getOriginalWidth() * Float.parseFloat(preferences.getString("max_zoom",  Float.toString(Constants.DEFAULT_MAX_ZOOM_FACTOR)));
  }


  private float getTheMaxHeight()
  {
    SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getContext());
    return getOriginalHeight() * Float.parseFloat(preferences.getString("max_zoom",  Float.toString(Constants.DEFAULT_MAX_ZOOM_FACTOR)));
  }
  ...
  {
    return width > getTheMaxWidth() || height > getTheMaxHeight();
  }
   ...
   * 
   * @param frame
   *          Frame of the image to scroll to.
   * @param keepInside
  ...
  /**
   * Recalculates the given frame of the original image considering the given width as the width of
   * the image. Also centers the new frame in the parent view
   * 
   * @param frame
  ...
  /**
   * Scales the view by the given factor without assuming anything of the size of the view. Scaling
   * is performed based on the original size of the drawable. Like all other scale methods, scroll
   * is positioned in the initial position for reading.
   * 
   * @param factor
   */
  public void scaleByFactor(float factor)
  {
    ...
    {
      newWidth = Math.round(getTheMaxWidth());
      newHeight = Math.round(getTheMaxHeight());
      factor = (float) newWidth / (float) getWidth();
      setScaleType(ScaleType.FIT_CENTER);
      setLayoutParams(createLayoutParams(newWidth, newHeight));
      
      SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getContext());
      zoomFactor = Float.parseFloat(preferences.getString("max_zoom",  Float.toString(Constants.DEFAULT_MAX_ZOOM_FACTOR)));
    }
  ...
  {
    SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getContext());
    float zoom_step = Float.parseFloat(preferences.getString("zoom_step",  Float.toString(Constants.DEFAULT_ZOOM_STEP)));
    float factor = (float) Math.pow(zoom_step, increment);
  ...
  public void zoom(float factor, Point imagePoint)
  {
    ...
    if (imagePoint != null)
    { // Scroll to point before zooming
